...
| `LLM_RETRIES` | `0` | 同一模型单轮内的重试次数；`0` = 不重试（每个模型仅尝试一次，失败即切换模型/轮次） |
...
LLM 汉化 (scripts/translate.py)
  ...
  │       每模型单轮内尝试 1 + LLM_RETRIES 次（默认 0 = 每个模型仅尝试一次）
  ├─ 结构校验（中文存在、关键内容保留、行数合理、结尾一致）
  ├─ README 头部插入来源声明 + how2use.md 使用说明指引
  └─ 输出 UTF-8 BOM 文件
        │
        ├─ ps1 或 README 任一汉化失败 → SMTP 告警邮件（可选）→ 任务失败，不发布
        ▼
...
> **失败即不发布**：ps1 脚本与 README 文档的汉化在同一个步骤中执行，
> 任一汉化失败都会使任务失败（可选 SMTP 告警），**不会提交、不会创建 Release**。
> 只有两者全部汉化成功才会提交并发布。
