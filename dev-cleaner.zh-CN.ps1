...
# 报告的尺寸求和并像其他估算一样传入 Format-Size。后缀顺序很重要：kB/MB/GB/TB
# 都以 "B" 结尾，因此裸 "B" 情况必须最后检查。Docker 始终使用点号作为小数
...
# -交互式选项通过二次提示提供更深度的 `prune -af`。"清理全部缓存"
# 不带此选项调用，因此批量运行不会意外删除带标签的镜像。
function Clear-Docker {
    ...
    # `prune -f` 删除已停止的容器、未使用的网络、悬空（未标记）镜像和
    # 未使用的构建缓存；-f 跳过 Docker 自身的确认（脚本已确认）。`-a`
    # 还会删除容器未使用的 EVERY 镜像，包括可能仅存在于私有注册表的
    # 带标签镜像——因此通过提示选择启用，不在批量"清理全部缓存"路径中。
    # (--volumes 在两种情况下都省略：它会删除命名卷数据如数据库。)
    $pruneArgs = @('-f')
    if ($Interactive) {
        Write-Host ""
        Write-Host "是否也删除未使用但带标签的镜像？可释放更多空间，但需要重新拉取/重新构建（例如私有注册表镜像）。(y/N):" -ForegroundColor Yellow
        $ans = Read-Host
...
function Invoke-EstimateAll {
    ...
    # 可回收的构建缓存来自 `system df` 摘要（Docker 正确计算它；只有其 Images 数据在 containerd 镜像存储下不可靠）。每个镜像的尺寸来自 `system df -v`，
...
交互式菜单:
  选项 99 估算每个条目的可回收空间，并使用 "(估算: <大小>)" 重新绘制菜单。
