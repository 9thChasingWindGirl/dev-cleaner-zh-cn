...
# 例如 "1.02GB"、"728.5MB"、"32.8kB"、"0B"）转换为字节，使 Docker 自身
# 报告的尺寸可以求和并传给 Format-Size，与其他所有估算一致。后缀顺序很重要：
# kB/MB/GB/TB 都以 "B" 结尾，因此裸 "B" 的情况必须最后检查。Docker 始终使用
# 点号作为小数分隔符，因此使用 InvariantCulture。只读。
...
# 为稳定的分类键返回 " (估算: <标签>)"，如果尚未计算则返回 ""。
# 键为字符串（而非菜单编号），因此重新编号时保持安全。
function Get-Est {
    param([string]$Key)
...
function Clear-VisualStudio {
    ...
    Write-Item "✓" "Green" "正在清理来自以下路径的 Visual Studio 项目: $SearchDir"
    ...
    if ($cleanedCount -gt 0) {
        ...
        Write-Item "ℹ️" "Yellow" "在以下路径中未找到 Visual Studio 项目: $SearchDir"
    }
...
function Clear-Flutter {
    ...
    Write-Item "✓" "Green" "正在递归清理来自以下路径的 Flutter 项目: $SearchDir"
    ...
    if ($cleanedCount -gt 0) {
        ...
        Write-Item "ℹ️" "Yellow" "在以下路径中未找到 Flutter 项目: $SearchDir"
    }
...
function Clear-PlatformIO {
    ...
    foreach ($file in $platformioFiles) {
        ...
        Write-Host "  正在 $projectDir 中运行 pio clean" -ForegroundColor Cyan
...
function Clear-Cordova {
    ...
    if (Test-Path $cordovaPath) {
        ...
        # Cordova 会在 lib\tmp* 下留下过期的 npm tarball/解压文件
...
function Clear-Electron {
    ...
    if (Test-Path $electronPath) {
        ...
        # 缓存的预构建二进制文件；清除内容，保留 Electron 期望的目录
...
# Claude Code 的原生安装程序会将它曾经安装的每个版本都保留在
# versions\ 目录下，且不会删除旧版本（每个版本约 190 MB）。与此脚本中其他
# 非缓存目录的目标不同：其中一个文件是 `claude` 命令实际运行的二进制文件。
# Windows 上没有符号链接可解析（.local\bin\claude.exe 是所用版本的副本），
# 因此通过大小识别活跃版本，然后再用 SHA256 验证。只读。由 Clear-ClaudeCode
# 和 Invoke-EstimateAll 共享，以确保估算和清理保持一致。
...
function Get-ClaudeRemovableVersions {
    ...
    # 仅对与启动器大小完全相同的候选文件进行哈希计算：对不可能匹配的
    # 200 MB 二进制文件进行哈希纯粹是 I/O 开销。
    ...
    # 没有匹配意味着启动器不是这些二进制文件之一：用户用自己的替代了它，
    # 就像 macOS/Linux 上的非符号链接启动器一样。无法识别正在使用的版本，因此跳过。
    if (-not $active) {
        return [PSCustomObject]@{ Status = 'unknown'; ActiveName = $null; Files = @(); InUse = @() }
    }

    # 同时保留最新的文件：更新可能写入 versions\ 而不会复制到 bin\，
    # 因此最新的文件可能确实不是当前运行的版本。
    $newest = $files | Sort-Object LastWriteTime -Descending | Select-Object -First 1

    # 较旧会话仍在运行的二进制文件。$_ .Path 对其他用户的进程会抛出访问被拒绝，
    # 而在管道内部抛出的异常会中止整个管道并静默跳过其后的所有进程，
    # 因此保护逻辑置于脚本块内，逐元素执行，而非围绕整个管道。
...
# 最新版本，以及任何仍在运行的会话持有的版本。
# 从不触碰 %USERPROFILE%\.claude\ 或 .claude.json：设置、MCP 配置和会话历史位于那里。
function Clear-ClaudeCode {
    ...
    if ($info.InUse.Count -gt 0) {
        Write-Item "→" "Cyan" "Claude Code：仍有 $($info.InUse.Count) 个版本正在运行；已保留。"
    }
...
# -Interactive 选项通过二次提示提供更深层的 `prune -af`。"清理全部缓存"
# 不带此参数调用，因此批量运行不会意外删除已标记的镜像。
function Clear-Docker {
    ...
    # `Get-Command` 仅证明 CLI 存在；守护进程可能仍已停止。
    ...
    # -f 跳过 Docker 自身的确认（脚本已确认）。`-a` 额外删除容器未使用的
    # 所有镜像，包括可能仅存在于私有注册表的已标记镜像——因此通过提示选择启用，
    # 绝不包含在批量"清理全部"路径中。（--volumes 在两种情况下都省略：它会删除
    # 命名卷数据，如数据库。）
    $pruneArgs = @('-f')
    if ($Interactive) {
        Write-Host ""
...
function Invoke-EstimateAll {
    ...
    if ($claudeInfo.Status -eq 'unknown') {
        # 无法测量，因此不加入字节总数，与下面的 Docker "守护进程未运行" 情况类似。
    ...
    #   -af ：上述内容 + 容器未使用的所有镜像（包括已标记的）
    # 可回收的构建缓存来自 `system df` 摘要（Docker 计算正确；仅其 Images 数值
    ...
    if (Get-Command docker -ErrorAction SilentlyContinue) {
        ...
        if ($LASTEXITCODE -eq 0 -and $dfFmt) {
            ...
            # 解析 "Images space usage:" 表。CREATED 是多词字段
...
function Show-Menu {
    ...
    Write-Host ("11. 清理 Docker（清理容器、悬空镜像及构建缓存；在删除未使用的已标记镜像前会询问）" + (Get-Est 'docker')) -ForegroundColor Green
    ...
    Write-Host ("14. 清理 Windows 临时文件与回收站" + (Get-Est 'windowstemp')) -ForegroundColor Green
    Write-Host ("15. 清理浏览器缓存（Chrome、Edge、Firefox、Brave、Opera）" + (Get-Est 'browser')) -ForegroundColor Green
    Write-Host ("16. 清理应用缓存（Slack、Teams、Discord、Spotify、WhatsApp）" + (Get-Est 'appcontainers')) -ForegroundColor Green
    Write-Host ""
    ...
    while ($true) {
        ...
        switch ($choice) {
            ...
            "2" {
                ...
                Write-Host ""
                Write-Host "输入自定义目录路径，或按 Enter 使用当前设置:" -ForegroundColor Yellow
            ...
            "99" {
                ...
                Invoke-EstimateAll
                # 只读：跳过前后摘要，并以 freshly 计算的估算值重新绘制菜单。
                continue
            }
            default {
                Write-Host "无效选择。请输入 0 到 16 之间的数字。" -ForegroundColor Red
...
if ($Help) {
    Write-Host @"
开发环境清理工具 v$SCRIPT_VERSION
一款用于 Windows 开发环境的强大清理工具
...
交互式菜单:
  选项 99 估算每个条目的可回收空间，并以 "(估算: <大小>)" 形式重新绘制菜单。
  此操作为只读（不会删除任何内容）。前缀 "~" 表示近似值；Flutter、
...
示例:
  ...
  .\dev-cleaner.ps1 -FlutterDir "D:\Projects" -VsDir "D:\VS"  # 两者均自定义
...
if ($Version) {
    ...
    Write-Host "一款用于 Windows 开发环境的强大清理工具"
    Write-Host "仓库: $GITHUB_REPO"
    exit 0
}

# 确定 Flutter 搜索目录（优先级：CLI > 环境变量 > 默认）
...
# 确定 Visual Studio 搜索目录（优先级：CLI > 环境变量 > 默认）
$script:VsSearchDir = "."
...
# 初始确认
Clear-Host
Write-Host "--- 开发环境清理工具 ---" -ForegroundColor Red
Write-Host "此脚本将永久删除您系统中的缓存文件。"
Write-Host "请在继续之前仔细查看各选项。"
Write-Host ""
