...
# Docker 自报的尺寸求和并馈送给 Format-Size，与其他所有估算一致。
# 后缀顺序很重要：kB/MB/GB/TB 都以 "B" 结尾，因此裸 "B" 的情况必须最后检查。
# Docker 始终使用点号作为小数分隔符，因此使用 InvariantCulture。只读。
...
function Clear-VisualStudio {
    ...
    Write-Item "✓" "Green" "正在从以下位置清理 Visual Studio 项目：$SearchDir"
    ...
    if ($cleanedCount -gt 0) {
        ...
        Write-Item "ℹ️" "Yellow" "在以下位置未找到 Visual Studio 项目：$SearchDir"
    }
...
function Clear-Flutter {
    ...
    Write-Item "✓" "Green" "正在从以下位置递归清理 Flutter 项目：$SearchDir"
    ...
    if ($cleanedCount -gt 0) {
        ...
        Write-Item "ℹ️" "Yellow" "在以下位置未找到 Flutter 项目：$SearchDir"
    }
...
function Clear-BrowserCaches {
    ...
    if (Test-Path $firefoxProfiles) {
        ...
        Write-Item "✕" "Yellow" "Firefox 缓存未找到。已跳过。"
    }
}
...
function Clear-Cordova {
    ...
    if (Test-Path $cordovaPath) {
        ...
        # Cordova 会在 lib\tmp* 下遗留过时的 npm tarball/解压文件
...
# Claude Code 的原生安装程序会将每个版本都保留在 versions\ 目录下，
# 且永远不会删除旧版本（每次发布约 190 MB）。与本脚本中其他目标不同：
# 其中一个文件是 `claude` 命令实际运行的二进制文件。
# Windows 上没有符号链接可解析（.local\bin\claude.exe 是被使用版本的副本），
# 因此通过文件大小识别活跃版本，然后比对 SHA256。
# 只读。由 Clear-ClaudeCode 和 Invoke-EstimateAll 共享，以确保估算和清理不会偏离。
...
function Get-ClaudeRemovableVersions {
    ...
    # 仅对与启动器大小完全匹配的文件进行哈希计算：对不可能匹配的 200 MB 二进制文件
    # 进行哈希纯粹是 I/O 开销。
    ...
    # 未匹配意味着启动器不是这些二进制文件之一：用户已用自己的启动器替换，
    # 类似于 macOS/Linux 上的非符号链接启动器。无法识别正在使用的版本，因此跳过。
    if (-not $active) {
        return [PSCustomObject]@{ Status = 'unknown'; ActiveName = $null; Files = @(); InUse = @() }
    }

    # 同时保留最新版本：更新可能写入 versions\ 而未复制到 bin\，
    # 因此最新版本可能 legitimately 不是当前运行的版本。
    $newest = $files | Sort-Object LastWriteTime -Descending | Select-Object -First 1

    # 旧会话仍在运行的二进制文件。$_．Path 对其他用户的进程会抛出"访问被拒绝"，
    # 且管道内抛出的异常会中断管道并静默跳过后续所有进程，因此保护块
    # 放在脚本块内部、逐个元素，而不是环绕整个管道。
...
# -Interactive 通过二次提示提供更深入的 `prune -af`。"清理全部缓存"
# 不带此参数调用，因此批量运行时不会意外删除带标签的镜像。
function Clear-Docker {
    ...
    # `Get-Command` 仅证明 CLI 存在；守护进程可能仍为关闭状态。
    ...
    # `prune -f` 删除已停止的容器、未使用的网络、悬空（未打标签）的镜像
    # 和未使用的构建缓存；-f 跳过 docker 自身的确认（脚本已确认）。
    # `-a` 额外删除所有未被容器使用的镜像，包括可能仅存在于私有注册表的
    # 带标签镜像——因此通过提示进行可选操作，不会在批量"清理全部缓存"路径中。
    # （--volumes 在两种情况下均省略：它会删除命名卷数据，如数据库。）
    $pruneArgs = @('-f')
    if ($Interactive) {
        Write-Host ""
        Write-Host "是否删除未使用但已打标签的镜像？可释放更多空间，但需要重新拉取/重新构建" -ForegroundColor Yellow
...
# --- 可回收空间估算 ---
# 只读：计算每个清理选项将要删除的当前磁盘大小，然后 Show-Menu 在
# 每个条目旁显示它。分类使用稳定的字符串键，以便安全地重新编号菜单。
# 重要：请保持这些路径列表与上方的 Clear-* 函数同步。
...
function Invoke-EstimateAll {
    ...
    if ($claudeInfo.Status -eq 'unknown') {
        # 无法测量，因此不加入字节总数，与下方 Docker "守护进程未运行" 情况相同。
    ...
    # Docker 不是基于路径的：直接向 docker 询问（只读）。两个数值，
    # 因为选项 11 提供两种 prune 模式：
    #   -f  ：可回收的构建缓存 + 悬空（未打标签）镜像
    #   -af ：上述 + 所有未被容器使用的镜像（包括带标签的）
    # 可回收的构建缓存来自 `system df` 摘要（Docker 计算正确；其 Images 数值
    # 在 containerd 镜像存储下不可靠）。每个镜像的大小来自 `system df -v`，
    # 对 0 容器行的 UNIQUE SIZE 求和，以避免重复计算共享层。近似值，因此不加入字节总数。
...
function Show-Menu {
    ...
    Write-Host ("11. 清理 Docker（修剪容器、悬空镜像及构建缓存；在删除未使用但已打标签的镜像前会询问）" + (Get-Est 'docker')) -ForegroundColor Green
    ...
    while ($true) {
        ...
        switch ($choice) {
            ...
            "2" {
                ...
                Write-Host ""
                Write-Host "请输入自定义目录路径，或直接按 Enter 使用当前设置：" -ForegroundColor Yellow
            ...
            "99" {
                ...
                Invoke-EstimateAll
                # 只读：跳过前后对比摘要，并使用 freshly computed 的估算值重绘菜单
                continue
            }
            default {
...
交互式菜单：
  选项 99 会估算每个条目的可回收空间，并在菜单中显示
  "(Estimate: <size>)"。该选项为只读（不会删除任何内容）。
  开头的 "~" 标记表示近似值；Flutter、PlatformIO 和 Visual Studio 的
  估算仅涵盖全局缓存。
...
示例：
  ...
  .\dev-cleaner.ps1 -FlutterDir "D:\Projects" -VsDir "D:\VS"  # 同时指定两个自定义目录
...
Write-Host "⚠️ 此操作对已删除的文件是【不可逆】的。⚠️" -ForegroundColor Yellow
